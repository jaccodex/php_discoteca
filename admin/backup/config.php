...
foreach($CONF as $key=>$value) {
    if (array_key_exists($key,$validate)) {
        switch($validate[$key]) {
            case "int": if (!preg_match("/^[0-9]*$/i",$value)) echo "<div class=\"red\">'".array_search($key,$conf)."' ".C_WRONG_TYPE."</div>\n";
                break;
            case "float": if (!preg_match("/^[0-9]*\.?[0-9]*$/i",$value)) echo "<div class=\"red\">'".array_search($key,$conf)."' ".C_WRONG_TYPE."</div>\n";
                break;
            case "email": if ($value||$CONF['email_use']) {
                    foreach(explode(",",$value) as $value2) {
                        if (!preg_match("/^\ *[‰ˆ¸ƒ÷‹a-zA-Z0-9_-]+(\.[‰ˆ¸ƒ÷‹a-zA-Z0-9\._-]+)*@([‰ˆ¸ƒ÷‹a-zA-Z0-9-]+\.)+([a-z]{2,4})$/i",$value2)) {
