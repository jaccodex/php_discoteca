...
function PMBP_email_store($attachments,$backup_info) {
    ...
    foreach($all_emails as $email) {
        // verify email
        if (!preg_match("/^\ *[‰ˆ¸ƒ÷‹a-zA-Z0-9_-]+(\.[‰ˆ¸ƒ÷‹a-zA-Z0-9\._-]+)*@([‰ˆ¸ƒ÷‹a-zA-Z0-9-]+\.)+([a-z]{2,4})$/",$email)) {
...
function PMBP_get_backup_files() {
    ...
    while ($file=@readdir($handle)) {
        if ($file!="." && $file!=".." && preg_match("/\.sql$|\.gz$|\.sql$|\.zip$/",$file)) {
...
function PMBP_file_info($mode,$path) {
	...
	//cambio estas lineas al fallar ereg_replace en php 5.3
	//
