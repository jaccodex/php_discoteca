...
class log {
    ...
    public function __construct($data_dir)
    {
        ...
        $this->_log_file = $this->_data_dir . 'log_file_' . time() . '.txt';
        
        $this->_puntero_fichero_log= @ fopen($this->_log_file,"wt");
    ...
    public function registrarLineaLog($texto, $salto=true)
    {
        fputs($this->_puntero_fichero_log , $texto);
        
        if($salto)
        {
            fputs($this->_puntero_fichero_log , PHP_EOL);
        }
    }
